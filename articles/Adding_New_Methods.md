...
This vignette explains how to add new publication bias correction
methods to the `PublicationBiasBenchmark` package. In the following, we
will use the PET (Precision-Effect Test) method as an example.
...
`method-{METHOD_NAME}.R` in the `R/` directory. Implementation of these
three functions allows users to apply the method via the
[`run_method()`](https://fbartos.github.io/PublicationBiasBenchmark/reference/run_method.md)
function.

## File Structure and Naming

For a method called “PET”, you need to create a file named
`R/method-PET.R` containing three functions:
...
This is the core function that implements your statistical method. Here
...
- `data`: Data frame with `yi` (effect sizes), `sei` (standard errors),
  and `ni` (sample sizes).
...
If your method does not provide certain values (e.g., Bayes factor), use
`NA`.
...
- The framework handles errors automatically - your function can throw
  errors. The package will catch the errors and attach them to an empty
  output with `convergence = FALSE` and the error message in `note`.
...
The selected settings are passed to your main function as the `settings`
parameter. There, the main function can use these settings to adjust its
behavior. Once defined, these settings cannot be changed retrospectively
to ensure reproducibility and continuity of the benchmark.

For an example of a method that defines several settings we can examine
...
This function specifies additional columns that your method returns
...
The column names must match exactly the additional columns that your
...
results can be merged in case the method fails with an error.
