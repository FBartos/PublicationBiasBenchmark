...
$(document).ready(function() {
  // Comprehensive solution for DT in nested tabs
  
  // Function to reinitialize DataTables
  function reinitializeDataTables() {
    setTimeout(function() {
      // Find all DataTables and recalculate
      $('.dataTables_wrapper').each(function() {
        ...
        if (table) {
          try {
            ...
            // Ignore errors for tables that aren't ready
          }
        }
      });
    }, 200);
  ...
  // Handle all possible tab events for Bootstrap 4 and 5
  $(document).on('shown.bs.tab shown.bs.pill', 'a[data-toggle="tab"], a[data-toggle="pill"], button[data-bs-toggle="tab"], button[data-bs-toggle="pill"]', function (e) {
    reinitializeDataTables();
  });
  
  // Additional event handlers for more robust detection
  $(document).on('click', '.nav-tabs a, .nav-pills a, .nav-tabs button, .nav-pills button', function() {
    setTimeout(reinitializeDataTables, 300);
  });
  
  // Observer to watch for tab content becoming visible
  if (window.MutationObserver) {
    var observer = new MutationObserver(function(mutations) {
      mutations.forEach(function(mutation) {
        if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
          var target = $(mutation.target);
          if (target.hasClass('active') && target.hasClass('tab-pane')) {
            reinitializeDataTables();
          }
        }
      });
  ...
  setTimeout(function() {
    ...
    setTimeout(reinitializeDataTables, 1000);
  }, 500);
  
  // Handle window resize events
  $(window).on('resize', function() {
    setTimeout(reinitializeDataTables, 100);
  });
});
```
