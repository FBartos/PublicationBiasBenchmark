...
create_raincloud_plot(this_results, "bias", "Bias", ylim_range = if (common_scale) c(-0.5, 0.5), reference_line = if (common_scale) 0)
```

`r if (!common_scale) "Methods are compared using the average of repetition-wise ranks. Direct comparison using the average bias is not possible because the data generating mechanisms differ in the outcome scale. See a DGM-specific results (or subresults)." else "Values lower than -0.5 or larger than 0.5 are visualized as -0.5 and 0.5 respectively."`
...
create_raincloud_plot(this_results, "rmse", "RMSE", ylim_range = if (common_scale) c(0, 0.5), reference_line = if (common_scale) 0)
```

`r if (!common_scale) "Methods are compared using the average of repetition-wise ranks. Direct comparison using the average rmse is not possible because the data generating mechanisms differ in the outcome scale. See a DGM-specific results (or subresults)." else "Values larger than 0.5 are visualized as 0.5."`
