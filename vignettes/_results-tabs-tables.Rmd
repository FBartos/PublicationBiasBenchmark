...
Coverage is the proportion of simulation runs in which the 95% confidence interval contained the true parameter value. Ideally, this value should be close to the nominal level of 0.95.
...
The Interval Score measures the accuracy of an uncertainty interval by combining its width and coverage. It penalizes intervals that are too broad or that fail to include the true value. A lower interval score indicates a better method.
...
tables_conditional_combined <- data.frame(
  Rank_Convergence   = this_rankings_conditional$combined_rank[order(this_rankings_conditional$combined_rank)],
  Method_Convergence = paste0(this_rankings_conditional$Method, "-", this_rankings_conditional$Setting)[order(this_rankings_conditional$combined_rank)],
  Value_Convergence  = this_rankings_conditional$combined_value[order(this_rankings_conditional$combined_rank)],
  Rank_Replacement   = this_rankings_replacement$combined_rank[order(this_rankings_replacement$combined_rank)],
  Method_Replacement = paste0(this_rankings_replacement$Method, "-", this_rankings_replacement$Setting)[order(this_rankings_replacement$combined_rank)],
  Value_Replacement  = this_rankings_replacement$combined_value[order(this_rankings_replacement$combined_rank)]
)
# Create table with two-level headers
