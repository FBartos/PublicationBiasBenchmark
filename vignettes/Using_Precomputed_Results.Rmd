...
The package includes precomputed results for several DGMs. See the [Adding New DGMs](Adding_New_DGMs.html) vignette for details on the individual DGMs and their simulation designs.
...
You only need to download them once, unless you want to update to a newer version.
...
To retrieve all results across all conditions, methods, and repetitions, simply omit the filtering arguments:
...
You can filter results by specific methods or method settings:
...
# Retrieve results for a specific method setting
rma_reml_results <- retrieve_dgm_results(
  dgm = "Stanley2017",
  method = "RMA",
